...
Finally, note that from the theory of Koszul (pr)operads it is well-known that $\IBL$ is Koszul dual to $\Frob$, i.e., $\IBL^! = \Frob$ and $\Frob^! = \IBL$ (the coproduct is obtained from the product by dualization). Therefore, it holds $\IBLInfty = \Omega(\Frob^*)$, where $\Omega$ denotes the cobar construction and ${}^*$ is the linear dual; this precisely reflects our situation of having a Frobenius structure on $\HDR$ and $\IBLInfty$-structure on $\CDBCyc \HDR = \reallywidehat{\BCyc\HDR^*}$, since $\BCyc$ is a cyclic version of cobar construction.
